...
export type ServerAgentLoader = Pick<
  AgentLoader,
  'listAgents' | 'getAgentFile' | 'listLoadFailures'
>;
...
async function readReasoningEngineBody(
  ...
  return new Promise((resolve) => {
    // Decode through the stream's StringDecoder: a multi-byte UTF-8 sequence
    // straddling two chunks would otherwise become replacement characters on
...
export class AdkApiServer {
  ...
  private async init() {
    ...
    app.post('/run_sse', async (req: Request, res: Response) => {
      ...
      const events = this.runAgentEvents(res, request, {
...
+-----------------------------------------------------------------------------+`);
  ...
  private async *runAgentEvents(
    ...
    try {
      // The agent file is borrowed from the loader, which owns it and hands
      // the same instance to the next request, so it is not disposed here.
      const agentFile = await this.agentLoader.getAgentFile(request.appName);
      const loaded = await agentFile.load();
