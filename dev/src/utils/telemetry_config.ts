...
import {isFileNotFoundError, isRecord} from './file_utils.js';
 ...
 *   object, and `undefined` when the file is absent or cannot be parsed. An
 *   absent file is the ordinary case, so only a parse failure is logged.
 */
...
export function readTelemetryConsent(): boolean | undefined {
  const value = readConfig(getUserConfigPath())?.[TELEMETRY_KEY];
