...
// allowOrigins is a comma-separated list) so they can't be restricted to
// the plain-identifier token above. They only reach the Dockerfile's CMD
// line, so a newline in any of them still breaks out of that instruction
// the same way appName does, and once inside the CMD line they're read by
// /bin/sh at container start, so shell metacharacters must be neutralized
// too.
