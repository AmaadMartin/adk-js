...
describe('AgentLoader', () => {
  ...
  describe('AgentFile', () => {
    ...
    // An omitted `bundle` inherits the default instead, which is `true`; see
    // 'inherits the bundle default when only compile is given'.
    it('omits esbuild "external" when bundle is explicitly undefined', async () => {
      const agentPath = path.join(tempAgentsDir, 'agent2.ts');
      await fs.writeFile(agentPath, agent2TsContent);
