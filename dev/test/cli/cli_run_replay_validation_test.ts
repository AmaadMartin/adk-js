...
// Only `AgentFile` is replaced. `resolveAgentLocation` stays real, because the
// run names the app after the agent directory and asks that function for it.
...
describe('adk run --replay input validation', () => {
  ...
  it('replays the queries of a valid file', async () => {
    ...
    // A bare agent is served under the name of its directory, so that is the
    // app the replayed session belongs to.
