    ...
    getClient(): Promise<unknown> {
      return Promise.resolve({
        getRequestHeaders(): Promise<Headers> {
          return Promise.resolve(new Headers({Authorization: 'Bearer test'}));
        },
        request(): Promise<{data: {projectId: string}}> {
          return Promise.resolve({data: {projectId: 'test-project'}});
        },
      });
    }
  },
 ...
 *
 * The reader drains into the OTLP exporter `getGcpExporters` builds for
 * telemetry.googleapis.com, which is where this branch sends every signal.
 */
vi.mock('@opentelemetry/exporter-metrics-otlp-http', async () => {
  const {ExportResultCode} = await import('@opentelemetry/core');
  return {
    OTLPMetricExporter: class implements PushMetricExporter {
      export(
