...
describe('runIntegrationTests', () => {
  let logSpy: ReturnType<typeof silenceLogger>['info'];
  let errorSpy: ReturnType<typeof silenceLogger>['error'];
  ...
  beforeEach(() => {
    ...
    logSpy = logSpies.info;
    errorSpy = logSpies.error;
    run = spyOnRun();
  });
  ...
  const runAll = () => runIntegrationTests(OPTIONS);
