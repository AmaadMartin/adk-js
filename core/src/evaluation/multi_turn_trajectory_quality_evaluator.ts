 ...
 *
 * Unlike multi-turn task success (which only concerns whether the goal was
 * achieved), this metric cares about the path the agent took to achieve the
 * goal. This is a reference-free metric. It delegates to the Vertex Gen AI Eval
 * SDK via {@link MultiTurnVertexAiEvalFacade}; the `V1` suffix conveys that
 * other versions of the metric may exist and use a different strategy.
 *
 * Value range of the metric is [0, 1], with values closer to 1 being more
 * desirable.
 */
export class MultiTurnTrajectoryQualityV1Evaluator extends Evaluator {
  ...
  override evaluateInvocations(
    ...
    return new MultiTurnVertexAiEvalFacade(
      ...
      RubricMetric.MULTI_TURN_TRAJECTORY_QUALITY,
    ).evaluateInvocations(
      actualInvocations,
      expectedInvocations,
