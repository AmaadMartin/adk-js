...
import {EvalStatus, validateInvocationLengths} from './evaluator.js';
...
/**
 * Reads the tool use trajectory of an invocation.
 *
 * `intermediateData` holds either shape of `IntermediateDataType`. Only the
 * `IntermediateData` shape records a tool use trajectory, so the event shape
 * reads as an empty one, the same as an absent `intermediateData`.
 */
function getToolUses(invocation: Invocation): FunctionCall[] {
