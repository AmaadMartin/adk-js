...
/**
 * Builds the descriptor for one prebuilt metric.
 *
 * The interval defaults to `[0, 1]`, which every prebuilt metric but
 * `response_evaluation_score` scores in. Both ends are closed, because
 * {@link Interval} reads an absent `openAtMin` / `openAtMax` as closed.
 */
function metricInfo(
  metricName: PrebuiltMetrics,
  description: string,
  interval: Interval = {minValue: 0, maxValue: 1},
): MetricInfo {
  return {metricName, description, metricValueInfo: {interval}};
}
...
/**
 * Describes the two metrics `ResponseEvaluator` scores.
 *
 * One provider serves both metrics, so the metric it describes is chosen at
 * construction time.
 */
export class ResponseEvaluatorMetricInfoProvider implements MetricInfoProvider {
  ...
  /**
   * @throws {InputValidationError} when the metric name is neither
