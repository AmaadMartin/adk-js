...
/**
 * The session an eval case runs in.
 *
 * Names the app and user an eval conversation is replayed as, and optionally
 * pins a session id and the state that session starts from.
 */
export interface SessionInput {
  /** The name of the app the eval case runs under. */
  appName: string;

  /** The id of the user the eval case runs as. */
  userId: string;

  /**
   * The id of the session. When it names a session that already exists, that
   * session is reused and {@link state} is ignored.
   */
  sessionId?: string;
