...
const LATEST_TURN_USER_SIMULATOR_EVALUATOR_PROMPT_TEMPLATE = `You are a data scientist tasked with evaluating the quality of a User Simulator that is interacting with an Agent.
...
const LATEST_TURN_USER_SIMULATOR_WITH_PERSONA_EVALUATOR_PROMPT_TEMPLATE = `You are a data scientist tasked with evaluating the quality of a User Simulator that is interacting with an Agent.
Your task is to determine if the Generated User Response is consistent with:
  - The Conversation Plan: A list of high-level goals that the User Simulator is expected to achieve in the conversation.
  - The Conversation History: The exchange between the User Simulator and the Agent so far.
...
/** Matches a `{{ name }}` placeholder, capturing whatever it names. */
const PLACEHOLDER_PATTERN = /\{\{([^{}]*)\}\}/g;

/**
 * The values a template placeholder may name, keyed by placeholder name.
 *
 * A placeholder that names no key renders as the empty string. The map is the
 * whole vocabulary of the template language, so a persona field is safe to
 * render even though a data file or an agent authored it.
 */
type PromptValues = ReadonlyMap<string, string>;

/** How a per-turn user simulator quality prompt is filled in. */
export interface PerTurnUserSimulatorQualityPromptOptions {
  /** The plan the simulated user was told to follow. */
  conversationPlan: string;

  /** The conversation so far, as {@link formatConversationHistory} renders it. */
  ...
  /** The persona the simulated user adopted, when it had one. */
  userPersona?: UserPersona;
}

/**
 * Substitutes the named values into a template.
 *
 * The template language is substitution and nothing else: a placeholder is
 * replaced by the value it names, and every other expression renders as the
 * empty string. adk-python renders these templates with jinja2's
 * `SandboxedEnvironment`, which evaluates expressions and rejects the unsafe
 * ones. There is no comparable sandbox for a JavaScript template engine, so an
 * allowlist of names takes its place.
 *
 * The values are inserted, never re-scanned, so a value that itself looks like
 * a placeholder is left alone.
 */
function renderTemplate(template: string, values: PromptValues): string {
  return template.replace(
    PLACEHOLDER_PATTERN,
    (_match, name: string) => values.get(name.trim()) ?? '',
  );
}

/** Renders one behavior as the evaluation criteria section it contributes. */
function formatBehaviorCriteria(
  behavior: UserBehavior,
  values: PromptValues,
): string {
  const violationRubrics = behavior.violationRubrics
    .map((rubric) => `  * ${rubric}`)
    .join('\n');

  return (
    `\n## Criteria: ${renderTemplate(behavior.name, values)}\n` +
    `${renderTemplate(behavior.description, values)}\n\n` +
    'Mark as FAIL if any of the following Violations occur:\n' +
    `${renderTemplate(violationRubrics, values)}\n`
  );
}

/**
 * Returns the prompt that asks a judge model whether one simulated user turn
 * is consistent with the scenario it was produced from.
 *
 * The persona template is used when a persona is given, and the plain one
 * otherwise.
 */
export function getPerTurnUserSimulatorQualityPrompt(
  options: PerTurnUserSimulatorQualityPromptOptions,
): string {
  const values = new Map<string, string>([
    ['conversation_plan', options.conversationPlan],
    ['conversation_history', options.conversationHistory],
    ['generated_user_response', options.generatedUserResponse],
  ...
  values.set('persona.description', persona.description);
  // Built before it joins the vocabulary, so a persona field cannot name it.
  const personaCriteria = persona.behaviors
    .map((behavior) => formatBehaviorCriteria(behavior, values))
    .join('');
  values.set('persona_criteria', personaCriteria);
