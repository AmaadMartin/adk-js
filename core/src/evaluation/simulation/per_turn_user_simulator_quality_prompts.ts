...
/** The judge instructions used when the scenario names a persona. */
...
Mark as FAIL if any of the following Violations occur:
{{ b.violation_rubrics_str | render_string_filter}}
{% endfor %}
# Output Format
Format your response in the following JSON format:
{
...
/** Renders the judge prompts. Static, so one environment serves them all. */
const QUALITY_ENV = new nunjucks.Environment(null, {
  autoescape: false,
  throwOnUndefined: false,
});
QUALITY_ENV.addFilter('render_string_filter', renderStringFilter);

/** Returns the judge template that fits the scenario. */
function getQualityPromptTemplate(userPersona?: UserPersona): string {
...
/**
 * Gives each behavior the `violation_rubrics_str` the persona template reads.
 * adk-python's template calls a method here; the value is precomputed so that
 * no callable enters the render context.
 */
function toTemplatePersona(persona: UserPersona): Record<string, unknown> {
  return {
    ...
    behaviors: persona.behaviors.map((behavior) => ({
      ...behavior,
      violation_rubrics_str: behavior.violationRubrics
        .map((rubric) => `  * ${rubric}`)
        .join('\n'),
    })),
  };
 ...
 *
 * The conversation plan and history are bound as variables, never compiled,
 * because the agent under test writes part of the history.
 *
...
export function getPerTurnUserSimulatorQualityPrompt(params: {
  ...
  return QUALITY_ENV.renderString(
    getQualityPromptTemplate(params.userPersona),
    templateParameters,
  );
}
