 ...
 *
 * The discriminator selects {@link LlmBackedUserSimulator} when an eval config
 * names it. {@link LlmBackedUserSimulator} also accepts the settings without
 * the discriminator, so a caller can build one directly.
 */
...
export class LlmBackedUserSimulator implements UserSimulator {
  ...
  /**
   * @param params.config The settings of the simulator. A validated
   *   {@link LlmBackedUserSimulatorConfig} is one, and so is a settings object
   *   that names no `type`.
  ...
  constructor(params: {
    ...
    this.model = config.model ?? DEFAULT_USER_SIMULATOR_MODEL;
