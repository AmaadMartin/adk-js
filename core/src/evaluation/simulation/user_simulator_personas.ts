...
/** Renders one prompt bullet: two spaces, an asterisk, then the entry. */
function toBullets(entries: string[]): string {
 ...
 * @returns One indented bullet per instruction, or `''` for no instructions.
 */
export function getBehaviorInstructionsStr(behavior: UserBehavior): string {
  return toBullets(behavior.behaviorInstructions);
}
 ...
 * @returns One indented bullet per rubric, or `''` for no rubrics.
 */
export function getViolationRubricsStr(behavior: UserBehavior): string {
  return toBullets(behavior.violationRubrics);
}

/** An in-process registry of the personas an eval run can simulate. */
export class UserPersonaRegistry {
  // A Map, not an object: the ids come from the caller, and a Map has no
   ...
   * one persona can be registered under several ids.
   *
