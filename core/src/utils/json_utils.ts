 ...
 * string, so {@link toJsonSafe} replaces what cannot be represented, and
 * reports whether the replacement lost anything.
 *
 * {@link toJsonSerializable} above answers the same question one value at a
 * time. This one walks a whole record, keeps a `Map` and a `Set` readable
 * rather than writing `{}` for both, and returns a single `lossy` flag, which
 * is what a session service needs to warn once for a write.
 */
