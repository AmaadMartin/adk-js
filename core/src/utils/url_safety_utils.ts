...
/**
 * Parses `url` and returns it, requiring a well-formed `http`/`https` URL.
 * Throws for malformed URLs and for any other scheme. Performs no network
 * access.
 */
export function parseAllowedUrl(url: string): URL {
...
/**
 * Throws when `hostname` is a `localhost`-style name. Performs no network
 * access; pass the host through {@link normalizeHost} first.
 */
...
/**
 * Resolves `hostname` and throws if any resolved address is not globally
 * routable.
 *
 * Known limitation: this is a pre-flight lookup, so a caller that later
 * connects by hostname resolves the name a second time. A residual
 * time-of-check/time-of-use (DNS-rebinding) window exists between the two.
 * Closing it needs connection-level IP pinning in the caller.
 */
