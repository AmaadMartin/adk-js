...
import {toSnakeCasePropertyName} from './case_utils.js';
 ...
 * {@link toSnakeCasePropertyName} brings `X-Api-Key`, `apiKey` and `x_api_key`
 * to the same form, so one entry covers every spelling a payload might use.
 */
...
export function isSensitiveKey(key: string): boolean {
  const normalized = toSnakeCasePropertyName(key);
  return (
