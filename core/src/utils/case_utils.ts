 ...
 * and acronyms, so `REST API` becomes `rest_api`, `X-Trace-Id` becomes
 * `x_trace_id` and `HTTPResponse` becomes `http_response`. Repeated, leading
 ...
 *
 * The name the OpenAPI tools use for {@link snakeCase}. Both names convert one
 * name the same way.
 *
