...
export async function resolveAuthCredential(
  ...
  const credential = await loadCredential(ctx, invocationConfig);
  ...
  // The id is prefixed so the auth preprocessor stores the response instead of
  // trying to resume a function call.
  const requestId = `${TOOLSET_AUTH_CREDENTIAL_ID_PREFIX}${agentName}`;
