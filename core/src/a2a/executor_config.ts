 ...
 *
 * An event with no convertible parts produces no A2A event: a converter drops
 * a part by returning `undefined`, and expands it by returning an array. A
 * partial event reuses the artifact id its author is already streaming into,
 * and the final chunk releases that id.
 *
