...
export interface ListSessionsRequest {
   ...
   * When set, the order is total: sessions with an equal `lastUpdateTime`
   * are tie-broken by ascending session id in both directions, so paging
   * through the result is stable.
   */
  order?: 'asc' | 'desc';
}
