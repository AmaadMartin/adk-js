 ...
 *
 * @param createExporter Builds the exporter the reader drains into. The first
 *   caller decides it, and `getGcpExporters` is that caller: it hands over the
 *   OTLP exporter it already built for telemetry.googleapis.com, so the reader
 *   exports where every other signal does. It defaults to the Cloud Monitoring
 *   exporter, for a host that drives the reader without that setup.
 */
export function getAgentEngineMetricsSetup(
  createExporter: () => Promise<PushMetricExporter> = createGcpMetricExporter,
): Promise<MetricsState | undefined> {
  metricsSetup ??= buildAgentEngineMetricsSetup(createExporter);
  return metricsSetup;
}
...
async function buildAgentEngineMetricsSetup(
  createExporter: () => Promise<PushMetricExporter>,
): Promise<MetricsState | undefined> {
  if (!isAgentEngine()) {
    return undefined;
  }
  ...
  try {
    return buildRequestDrivenMetrics(await createExporter());
  } catch (e: unknown) {
    logger.warn(
