...
import {isActiveCacheMetadata} from './cache_metadata.js';
...
export async function isCacheValid(
  ...
  const cacheIntervals =
    cacheConfig.cacheIntervals ?? DEFAULT_CONTEXT_CACHE_CONFIG.cacheIntervals;
...
export class GeminiContextCacheManager {
  ...
  async handleContextCaching(llmRequest: LlmRequest): Promise<CacheMetadata> {
    ...
    if (isActiveCacheMetadata(previous)) {
  ...
  private async createNewCacheWithContents(
    ...
    const minTokens =
      cacheConfig.minTokens ?? DEFAULT_CONTEXT_CACHE_CONFIG.minTokens;
    if (previousPromptTokens < minTokens) {
      logger.debug(
  ...
  private async createGeminiCache(
    ...
    return tracer.startActiveSpan('create_cache', async (span) => {
      try {
        ...
        span.setAttribute('ttl_seconds', ttlSeconds);
        ...
        return {
          cacheName,
          expireTime:
            parseExpireTime(cachedContent.expireTime) ?? createdAt + ttlSeconds,
          fingerprint: await generateCacheFingerprint(
            llmRequest,
            cacheContentsCount,
            this.scope,
