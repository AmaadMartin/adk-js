  ...
  get apiClient(): GoogleGenAI {
    ...
    const options: GoogleGenAIOptions = this.vertexai
      ? {
          ...
          // An Express Mode key, when `geminiInitParams` resolved one. It and
          // the project/location pair are mutually exclusive, so only one of
          // them is set here.
          apiKey: this.apiKey,
  ...
  get liveApiClient(): GoogleGenAI {
    ...
    if (!this._liveApiClient) {
      const options: GoogleGenAIOptions = this.vertexai
        ? {
            vertexai: this.vertexai,
            // See `apiClient`: an Express Mode key excludes project and
            // location, so `location` stays unset rather than 'global'.
            apiKey: this.apiKey,
            project: this.project,
            location: this.location,
