...
import type {SampleAndScoreParams} from './sampler.js';
import {Sampler} from './sampler.js';

/** Captured metric scores are rounded to two decimals. */
...
function captureMetricResult(
  ...
  const {score} = evalMetricResult;
  return {
    metricName: evalMetricResult.metricName,
    score:
      score === undefined
        ? undefined
        : Math.round(score * SCORE_FACTOR) / SCORE_FACTOR,
...
export class LocalEvalSampler extends Sampler<LocalEvalSamplingResult> {
  ...
  override async sampleAndScore({
    ...
    const isTrain = exampleSet === Sampler.TRAIN_SET;
    const evalSetId = isTrain ? trainEvalSet : validationEvalSet;
    const evalCaseIds =
      batch ?? (isTrain ? trainEvalCaseIds : validationEvalCaseIds);
