...
/**
 * The parameters of an invocation, shared by {@link Runner.runAsync} and
 * {@link Runner.run}.
 */
export interface RunAsyncParams {
  ...
  runConfig?: RunConfig;
  /** An optional signal to abort the run. */
  abortSignal?: AbortSignal;
  /** Custom metadata to stamp on the appended user event. */
  customMetadata?: Record<string, unknown>;
  /**
   * Whether to yield the appended user event before any agent event. Off by
   * default.
   */
...
export class Runner {
  ...
  /**
   * Runs the agent, yielding the same events {@link runAsync} yields.
   *
   * Ported from `google/adk-python` `runners.py::Runner.run`, which exists so a
   * synchronous caller can drive the async runtime. TypeScript has no such
   * divide, so this is the convenience name only. Prefer {@link runAsync}, as
   * upstream's docstring does.
   *
   * @param params The parameters {@link runAsync} accepts.
   * @yields The events generated by the agent.
   */
  async *run(params: RunAsyncParams): AsyncGenerator<Event, void, undefined> {
    yield* this.runAsync(params);
  }
