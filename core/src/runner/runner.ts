...
import {createPartFromText, createUserContent, Modality} from '@google/genai';
...
import type {LiveRequestQueue} from '../agents/live_request_queue.js';
...
export class Runner {
   ...
   * @param params.stateDelta An optional state delta to apply to the session.
   * @param params.runConfig The run config for the agent.
   * @param params.customMetadata Metadata for the appended user event only; use
   *     {@link RunConfig.customMetadata} to stamp every event of the run.
   * @yields The events generated by the agent.
   */
      ...
      yield* runAsyncGeneratorInSpan<Runner, Event>(
        ...
        async function* () {
          ...
          if (newMessage) {
            ...
            if (invocationContext.runConfig?.saveInputBlobsAsArtifacts) {
...
/**
 * Merges run-level custom metadata into an event.
 *
 * Run-level keys are applied underneath the event's own `customMetadata`, so an
 * event that already defines a key keeps its own value. An absent or empty
 * {@link RunConfig.customMetadata} leaves the event untouched.
 */
function applyRunConfigCustomMetadata(
  event: Event,
  runConfig: RunConfig,
): void {
  const runMetadata = runConfig.customMetadata;
  if (!runMetadata || Object.keys(runMetadata).length === 0) {
    return;
  }
  event.customMetadata = {...runMetadata, ...event.customMetadata};
}
