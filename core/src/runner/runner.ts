...
export class Runner {
   ...
   *
   * The caller owns the {@link LiveRequestQueue} and uses it to push input
   * (content, realtime blobs, activity signals) into the running agent, and
   * closes it to end the live loop. Events produced by the agent are yielded to
   * the caller; non-partial events are persisted to the session exactly as in
   * {@link Runner.runAsync}.
   *
   * WARNING: Live mode is **experimental** and its API or behavior may change in
   * future releases.
   *
   * @param params.userId The user ID of the session.
   * @param params.sessionId The session ID of the session.
   * @param params.liveRequestQueue The queue the caller uses to send live
   *     requests to the agent. Required.
   * @param params.runConfig The run config for the agent. `streamingMode` is
   *     forced to {@link StreamingMode.BIDI} and `responseModalities` defaults to
   *     `[Modality.AUDIO]` when unset.
   * @param params.abortSignal An optional signal to abort the live run.
   * @yields The events generated by the agent.
   */
  async *runLive(params: {
    ...
    // Native audio models require a modality; default to AUDIO (parity with
    // adk-python Runner.run_live).
      ...
      yield* runAsyncGeneratorWithOtelContext<Runner, Event>(
        ...
        async function* () {
          ...
          if (beforeRunCallbackResponse) {
            ...
            // Step 2: Otherwise drive the live agent.
