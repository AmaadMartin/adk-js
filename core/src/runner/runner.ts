...
export class Runner {
   ...
   *     When omitted, no user event is appended, `stateDelta` is not applied,
   *     the agent is not invoked, and the generator completes without yielding
   *     any events. The session is still looked up, so a missing session still
   *     throws. This mirrors `Runner.run_async` in adk-python, where
   *     `new_message` is optional.
   * @param params.stateDelta An optional state delta to apply to the session.
   *     Only applied alongside a `newMessage`, since it rides on the appended
   *     user event.
   * @param params.runConfig The run config for the agent.
   * @yields The events generated by the agent.
   */
    ...
    try {
      yield* runAsyncGeneratorWithOtelContext<Runner, Event>(
        ...
        async function* () {
          ...
          // =========================================================================
          // Mirrors adk-python `Runner.run_async`, which only runs the
          // user-message callback when a message is present.
          const pluginUserMessage = newMessage
