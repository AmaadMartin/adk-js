...
export class Runner {
  ...
  /**
   * The context-caching policy the `App` declared, carried onto every
   * invocation this runner starts.
   */
  ...
  constructor(input: RunnerConfig) {
    ...
    this.contextCacheConfig = this.app.contextCacheConfig;
   ...
   *     resumable runner; a new invocation starts otherwise. Reconciled
   *     against the function responses in `newMessage` rather than trusted.
        ...
        async function* () {
          ...
          const existingUserContent =
            invocationId && !continuesPausedTask
              ...
              : undefined;
          // A run with no message of its own has only the recorded events to
          // work from, so the invocation it names must be in the session. A
          // caller that gives both an id and a message is naming the turn it
          // starts, which the session does not have to know yet.
          if (isResumedInvocation && !newMessage) {
