...
export interface RunnerConfig {
  ...
  /**
   * Default user ID for invocations, used whenever `runAsync` or
   * `runEphemeral` is called without an explicit `userId`. Every invocation
   * needs a user ID, so it must be supplied here or at every call site.
   */
  userId?: string;

  /**
   * Default session ID for invocations, used whenever `runAsync` is called
   * without an explicit `sessionId`. Every invocation needs a session ID, so
   * it must be supplied here or at every call site.
   */
  sessionId?: string;

  /**
   * Default run configuration for invocations, used whenever `runAsync` or
   * `runEphemeral` is called without an explicit `runConfig`.
   */
  runConfig?: RunConfig;
}
...
/**
 * Resolves a value that a Runner method accepts either as a call parameter or
 * from the Runner's internalized defaults, so both sources fail identically.
 *
 * @param value The value resolved from the parameter and instance defaults.
 * @param name The name of the value, used in the error message.
 * @param method The Runner method being invoked, used in the error message.
 * @returns The resolved value.
 * @throws If neither the call parameters nor the Runner supplied the value.
 */
function required(
  ...
  if (!value) {
    throw new Error(
      `${name} is required: provide it in ${method} params or configure it on Runner.`,
    );
  }
  return value;
}
...
export class Runner {
  ...
  constructor(input: RunnerConfig) {
    ...
    this.runConfig = input.runConfig;
  }

  /**
   * Runs the agent with a new, ephemeral session.
   *
   * @param params.userId The user ID of the session. Optional only because it
   *     falls back to {@link Runner.userId}; if neither is set this throws.
   ...
   * @param params.runConfig The run config for the agent. Falls back to
   *     {@link Runner.runConfig}, then to the built-in run config defaults.
   * @yields The Events generated by the agent.
   * @throws If no `userId` is available from either the params or the Runner.
   */
   ...
   * @param params.userId The user ID of the session. Optional only because it
   *     falls back to {@link Runner.userId}; if neither is set this throws.
   * @param params.sessionId The session ID of the session. Optional only
   *     because it falls back to {@link Runner.sessionId}; if neither is set
   *     this throws.
   * @param params.newMessage A new message to append to the session.
   * @param params.stateDelta An optional state delta to apply to the session.
   * @param params.runConfig The run config for the agent. Falls back to
   *     {@link Runner.runConfig}, then to the built-in run config defaults.
   * @yields The events generated by the agent.
   * @throws If no `userId` or no `sessionId` is available from either the
   *     params or the Runner.
   */
  async *runAsync(params: {
    ...
    const userId = required(params.userId ?? this.userId, 'userId', 'runAsync');
    const sessionId = required(
      params.sessionId ?? this.sessionId,
      'sessionId',
      'runAsync',
    );
    const {stateDelta} = params;
    const runConfig = createRunConfig(params.runConfig ?? this.runConfig);
