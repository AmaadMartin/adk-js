...
export class Runner {
        ...
        async function* () {
          ...
          if (newMessage) {
            ...
            if (invocationContext.runConfig?.saveInputBlobsAsArtifacts) {
              await this.saveArtifacts(invocationContext, newMessage);
              if (params.abortSignal?.aborted) {
                return;
              }
            }
  ...
  private async saveArtifacts(
    ...
    if (!invocationContext.artifactService || !message.parts?.length) {
      return;
    }
    ...
    for (let i = 0; i < message.parts.length; i++) {
      ...
      await invocationContext.artifactService.saveArtifact({
        filename: fileName,
        artifact: part,
      });
