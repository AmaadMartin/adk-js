...
import {appNameMismatchDetails} from '../agents/agent_origin.js';
...
export interface RunnerConfig {
  ...
  /**
   * Whether to create the session when it is not found. Defaults to `false`,
   * in which case a missing session raises {@link SessionNotFoundError}.
   */
  autoCreateSession?: boolean;
}

/**
 * The largest number of events {@link Runner.run} buffers ahead of its caller.
 * On reaching it the pump waits for the caller to drain the buffer; no event is
 * ever dropped.
 */
...
export class Runner {
  ...
  /** Whether a missing session is created instead of raising. */
  ...
  constructor(input: RunnerConfig) {
    ...
    this.appNameMismatch = appNameMismatchDetails(
      this.appName,
      isBaseAgent(this.agent) ? this.agent.adkOrigin : undefined,
    );
    if (this.appNameMismatch) {
      logger.warn(`App name mismatch detected. ${this.appNameMismatch}`);
    }
  }
   ...
   * @param params.stateDelta An optional state delta to apply to the session.
   * @param params.runConfig The run config for the agent.
   * @param params.yieldUserMessage Whether to yield the appended user event
   *     before any agent event. Off by default.
   * @yields The events generated by the agent.
   */
      ...
      yield* runAsyncGeneratorWithOtelContext<Runner, Event>(
        ...
        async function* () {
          ...
          const session = await this.getOrCreateSession({
            userId,
            sessionId,
            getSessionConfig: runConfig.getSessionConfig,
          });
  ...
  /**
   * Runs the agent ahead of its caller, buffering the events it produces.
   *
   * {@link runAsync} advances the invocation only when the caller asks for the
   * next event, so a slow consumer throttles the agent. `run` keeps the
   * invocation going while the caller is busy, buffering up to
   * `MAX_BUFFERED_RUN_EVENTS` events and then waiting for the caller to catch
   * up. The events and their order are exactly what `runAsync` produced.
   *
   * Ported from `google/adk-python` `runners.py::Runner.run`, which runs the
   * invocation on a background thread for the same reason. A caller that stops
   * iterating early gets no error, and the agent's failure is raised only after
   * the events it produced before failing have been yielded.
   *
   * @param params The same parameters {@link runAsync} accepts.
   * @yields The events generated by the agent.
   */
  async *run(params: {
    userId: string;
    ...
    yield* pumpEagerly(this.runAsync(params), MAX_BUFFERED_RUN_EVENTS);
  }

  /**
   * Loads the session, creating it when {@link autoCreateSession} is set.
   *
   * Ported from `google/adk-python`
   * `runners.py::Runner._get_or_create_session`. Both {@link runAsync} and
   * {@link runLive} resolve their session here, so the two paths cannot drift.
   *
   * @throws {Error} When the runner has no app name to search under.
  ...
  private async getOrCreateSession(params: {
    ...
    const session = await this.sessionService.getSession({
      ...
      config: params.getSessionConfig,
    });
    if (session) {
      return session;
    }
    ...
    throw new SessionNotFoundError(
      sessionNotFoundMessage(params.sessionId, this.appNameMismatch),
    );
  }
...
/**
 * Merges run-level custom metadata onto an event, in place.
 *
 * A key the event already carries keeps the event's value, so an agent can
 * override a run-level default. Ported from `google/adk-python`
 * `runners.py::_apply_run_config_custom_metadata`.
 */
function applyRunConfigCustomMetadata(
  event: Event,
  runConfig: RunConfig,
): void {
  if (!runConfig.customMetadata) return;
...
/**
 * The message a missing session raises, with the app-name mismatch folded in
 * when the runner found one.
 *
 * Ported from `google/adk-python`
 * `runners.py::Runner._format_session_not_found_message`.
 */
function sessionNotFoundMessage(sessionId: string, mismatch?: string): string {
  const message = `Session not found: ${sessionId}`;
  if (!mismatch) return message;
  return (
    `${message}. ${mismatch} Ensure the runner appName matches that ` +
    'directory or pass appName explicitly when constructing the runner. ' +
    'The mismatch prevents the runner from locating the session. To ' +
    'automatically create a session when missing, set autoCreateSession=true ' +
    'when constructing the runner.'
  );
}

/**
 * Re-yields everything `source` produces, driving it ahead of the consumer.
 *
 * The pump reads `source` as fast as it produces, buffering into a bounded
 * {@link AsyncQueue} and waiting for the consumer once the buffer reaches
 * `highWaterMark`. Nothing is dropped. A consumer that stops iterating closes
 * the queue, which releases the pump and closes `source`.
 */
async function* pumpEagerly<T>(
  source: AsyncGenerator<T, void, undefined>,
  highWaterMark: number,
): AsyncGenerator<T, void, undefined> {
  const queue = new AsyncQueue<T>({highWaterMark});
  const pump = (async () => {
    try {
...
/**
 * The error a failed run reports to its caller.
 *
 * A thrown `Error` is the agent's own failure and is re-thrown unchanged. Any
 * other thrown value is wrapped, because re-throwing it would tell the caller
 * something about its own control flow rather than about the agent — the
 * reason `google/adk-python` `runners.py::Runner.run` converts a
 * `BaseException` into a `RuntimeError`.
 */
function asRunFailure(thrown: unknown): Error {
  if (thrown instanceof Error) return thrown;
  const description = typeof thrown === 'string' ? thrown : typeof thrown;
  return new Error(`Agent run terminated by ${description}.`, {cause: thrown});
}
