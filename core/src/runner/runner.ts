...
export class Runner {
   ...
   * @param params.invocationId Set to resume an interrupted invocation.
   * @param params.newMessage A new message to append to the session. Omit it
   *     only when resuming an invocation on a resumable app.
   * @param params.stateDelta An optional state delta to apply to the session.
   * @param params.runConfig The run config for the agent.
   * @yields The events generated by the agent.
   */
        ...
        async function* () {
          ...
          if (!newMessage) {
            if (!params.invocationId) {
              throw new Error(
                'Running an agent requires either a newMessage or an ' +
                  'invocationId to resume a previous invocation. Session: ' +
            ...
            invocationContext = await this.setupContextForResumedInvocation({
              ...setup,
              invocationId: params.invocationId,
            });
          } else if (!isResumable) {
            invocationContext = await this.setupContextForNewInvocation({
              ...setup,
              newMessage,
              invocationId: params.invocationId,
            });
          } else {
            const resumedId = this.resolveInvocationId(
              session,
              newMessage,
              params.invocationId,
            );
            invocationContext = resumedId
              ? await this.setupContextForResumedInvocation({
                  ...setup,
                  newMessage,
                  invocationId: resumedId,
                })
              : await this.setupContextForNewInvocation({...setup, newMessage});
          }
  ...
  /**
   * Builds the invocation context for one run of the root.
   */
  private newInvocationContext(params: {
    session: Session;
    newMessage?: Content;
    runConfig: RunConfig;
    invocationId?: string;
    ...
    const {session, runConfig} = params;
  ...
  /** Builds the context for an invocation that starts now. */
  private async setupContextForNewInvocation(params: {
    session: Session;
    newMessage: Content;
    runConfig: RunConfig;
    stateDelta?: Record<string, unknown>;
    ...
    const invocationContext = this.newInvocationContext(params);
    await this.handleNewMessage({...params, invocationContext});
    this.resolveAgentForContext(invocationContext);
    return invocationContext;
  }

  /**
   * Builds the context for an invocation that is being resumed.
   *
   * adk-python also replays the recorded agent states here and returns early
   * when the resumed agent already finished. `InvocationContext` carries
   * neither in this SDK, so those two steps are absent: agent-state resumption
   * is a separate subsystem that has not been ported.
   */
  private async setupContextForResumedInvocation(params: {
    session: Session;
    newMessage?: Content;
    invocationId: string;
    runConfig: RunConfig;
    ...
    const {session, invocationId} = params;
    if (!session.events.length) {
      throw new Error(`Session ${session.id} has no events to resume.`);
    }
  ...
  /**
   * Points the context at the agent that should continue the session.
   *
   * Only meaningful for an agent root: this resolves an event author against
   * the agent tree, and a node subtree is not in that tree.
   */
  private resolveAgentForContext(invocationContext: InvocationContext): void {
  ...
  /**
   * Runs the user-message plugin callback and appends the message to the
   * session.
   *
   * An invocation records exactly one opening user turn. When one is already
   * recorded, a further plain message for the same invocation is dropped
   * rather than appended, which is what stops a resume duplicating the turn it
   * replays. The contents are not compared, so a caller that resumes with
   * different text loses it; the runner warns when that happens.
   */
  private async handleNewMessage(params: {
    newMessage: Content;
    ...
    const {invocationContext, runConfig} = params;
    const abortSignal = invocationContext.abortSignal;
    let newMessage = params.newMessage;

    const isFunctionResponse = Boolean(
      newMessage.parts?.some((part) => part.functionResponse),
    );
    const recordedUserMessage = isFunctionResponse
      ? undefined
      : findUserMessageForInvocation(
    ...
    if (recordedUserMessage) {
      logger.warn(
        `Dropping the new message for invocation ${
          invocationContext.invocationId
        }: it already recorded an opening user turn. Omit newMessage to ` +
          'resume it, or start a new invocation to send different text.',
      );
      return;
    }
  ...
  /** Appends the user message to the session with its optional state delta. */
  private async appendUserEvent(params: {
    invocationContext: InvocationContext;
    content: Content;
    stateDelta?: Record<string, unknown>;
    customMetadata?: Record<string, unknown>;
    ...
    const event = createEvent({
      invocationId: invocationContext.invocationId,
      author: 'user',
      actions: params.stateDelta
        ? createEventActions({stateDelta: params.stateDelta})
        : undefined,
  ...
  /**
   * The invocation this run continues, inferred from the function responses in
   * the new message when it carries any.
   *
   * @throws {Error} When a function response has no id, when no function call
   *     matches one, or when the responses span more than one invocation.
   */
  private resolveInvocationId(
    ...
    if (functionResponses.some((response) => !response.id)) {
      throw new Error(
        'Function response id is required to resume an invocation.',
      );
    }

    const resolved = resolveInvocationIdFromFr(session, newMessage);
    if (invocationId && resolved && invocationId !== resolved) {
      logger.warn(
        `Ignoring the provided invocationId ${invocationId}: the function ` +
          `responses in the new message belong to invocation ${resolved}.`,
      );
    }
    return resolved;
  }
