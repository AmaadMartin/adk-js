...
/** The payload {@link DEFAULT_TASK_INPUT_SCHEMA} describes. */
...
/** The payload {@link DEFAULT_TASK_OUTPUT_SCHEMA} describes. */
...
const defaultTaskInputSchema = z.strictObject({
  goal: z.string({error: 'goal must be a string.'}).optional(),
  background: z.string({error: 'background must be a string.'}).optional(),
});
 ...
 *
 * Both fields are optional in the reference, so `{}` is valid. Unknown keys
 * are rejected, matching the reference's `extra='forbid'`.
 *
 * @param value The value to validate.
 * @returns A frozen input payload.
...
export function parseDefaultTaskInput(value: unknown): DefaultTaskInput {
  return parseOrThrow(defaultTaskInputSchema, value);
}
 ...
 *
 * @param value The value to validate.
 * @returns A frozen output payload.
