 ...
 * HTTP round trips, and the sink is attached to the invocation, so it cannot
 * grow with the session. Exchanges past the cap are dropped.
 */
...
export interface HttpDebugExchange {
  ...
  url: string;
  /** Snake_case, because the record leaves the process on an invocation. */
  status_code: number;
  method: string;
 ...
 * @return A fetch that records what it sends and receives.
 */
