 ...
 *
 * It can also be configured with a prefix. If provided,
 * {@link BaseToolset.getToolsWithPrefix} prefixes the name of every discovered
 * tool with `${prefix}_`, and {@link MCPTool} sends the server-advertised name
 * when the model invokes it. `getTools` itself returns unprefixed tools, so a
 * string-array `toolFilter` lists the names the server advertises.
 *
 * Usage:
...
export class MCPToolset extends BaseToolset {
  ...
  async getTools(context?: ReadonlyContext): Promise<BaseTool[]> {
    ...
    if (Array.isArray(filter)) {
      // String-array filter: match against the name the server advertised.
