...
export class ToolConnectionAnalyzer {
  ...
  async analyze(tools: BaseTool[]): Promise<ToolConnectionMap> {
    ...
    try {
      // `parseToolConnectionMap` throws on an answer it cannot read, so the
      // degrade happens here instead of on an `undefined` return.
      return parseToolConnectionMap(parseFencedJson(responseText));
    } catch {
      logger.warn(
