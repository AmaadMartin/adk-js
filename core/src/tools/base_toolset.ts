...
export abstract class BaseToolset {
   ...
   *
   * The result is a promise, and the path is optional, to match
   * `BaseTool.fromConfig`: a toolset such as `MCPToolset` resolves its
   * connection params before it can construct itself, and one loader calls
   * every override the same way. This implementation is deliberately not
   * `async`, so an unimplemented `fromConfig` throws where it is called
   * instead of rejecting a promise nobody awaits.
   *
