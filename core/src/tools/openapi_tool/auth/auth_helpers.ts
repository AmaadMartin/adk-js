...
/**
 * Prefix of the tool argument that carries an injected auth value.
 *
 * The value is shared with the other ADK language implementations, so a tool
 * declaration generated by one of them stays readable by the others.
 */
...
/**
 * An OpenID Connect client, as the caller of an OpenAPI toolset owns it.
 *
 * This is the client configuration, not the provider's discovery document.
 */
...
/**
 * Converts a resolved credential into the parameter a tool call injects.
 *
 * @param authScheme The scheme the operation declares.
 * @param authCredential The credential that satisfies it, once exchanged.
 * @returns The parameter and the argument that fills it, or undefined when
...
export function credentialToParam(
  ...
  if (authScheme.type === 'apiKey' && authCredential.apiKey) {
    return apiKeyParam(authScheme, authCredential.apiKey);
  }

  if (authCredential.authType === AuthCredentialTypes.HTTP) {
    return httpParam(authScheme, authCredential);
  }

  // A service account, OAuth2 or OpenID Connect credential becomes a bearer
  // token once it is exchanged. Before that there is nothing to inject.
  if (authScheme.type === 'oauth2' || authScheme.type === 'openIdConnect') {
    const token = authCredential.http?.credentials?.token;
    return token ? bearerParam(authScheme, token) : undefined;
  }
...
function apiKeyParam(
  ...
  const param: ApiParameter = {
    ...
    name: INTERNAL_AUTH_PREFIX + authScheme.name,
    required: false,
  };
  return {param, kwargs: {[param.name]: apiKey}};
}
...
function httpParam(
  ...
  if (credentials?.token) {
    return bearerParam(authScheme, credentials.token);
  }
...
function bearerParam(authScheme: AuthScheme, token: string): CredentialParam {
  const param: ApiParameter = {
  ...
  return {param, kwargs: {[param.name]: `Bearer ${token}`}};
}
