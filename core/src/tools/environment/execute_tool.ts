...
/**
 * Error codes returned by {@link ExecuteTool}. The string values are part of
 * the tool's response contract and must remain stable.
 */
...
/**
 * Asks the client to approve `command` before it runs, and reports the answer.
 *
  ...
  if (!confirmation) {
    toolContext.requestConfirmation({
      hint:
        ...
        'requested to run the following command in the environment. Only ' +
