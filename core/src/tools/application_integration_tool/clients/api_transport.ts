...
import {parseServiceAccountKey} from '../../../utils/service_account_utils.js';
...
/**
 * Authenticated JSON transport for the Integration Connectors clients.
 *
...
export class ApiTransport {
  ...
  /**
   * Performs one authenticated JSON GET and returns the decoded body.
   *
  ...
  private async resolveClient(): Promise<AuthClient> {
    ...
    if (this.serviceAccountJson) {
      const key = parseServiceAccountKey(this.serviceAccountJson);
      this.client = new JWT({
    ...
    this.client = await new GoogleAuth({
      scopes: [CLOUD_PLATFORM_SCOPE],
    }).getClient();
    return this.client;
  }
