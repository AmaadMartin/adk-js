...
export class IntegrationConnectorTool extends BaseTool {
   ...
   *
   * A host calls this once it has exchanged the end-user credential, so that
   * the stored tool keeps the raw one and a later exchange starts from it
   * rather than from a token that has since expired.
   */
  @experimental
