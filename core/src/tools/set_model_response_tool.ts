...
/**
 * Creates the same tool as a {@link FunctionTool} over a genai `Schema`, with
 * validation supplied by the caller.
 *
 * {@link SetModelResponseTool} is the richer form: it takes the schema as the
 * agent declared it, so it can also carry a list or a record schema, which the
 * GenAI API cannot accept as parameters directly. Prefer it. This factory suits
 * a caller that already holds a converted genai `Schema` and its own validator.
 *
