 ...
 *
 * `ComputerState.screenshot` is optional, so this takes the narrower state
 * that {@link isComputerState} guarantees: the one that carries the bytes.
 */
function toStateResponse(
  state: ComputerState & {screenshot: Uint8Array},
): Record<string, unknown> {
  return {
    image: {
      mimetype: 'image/png',
