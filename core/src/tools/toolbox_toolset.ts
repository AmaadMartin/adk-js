...
export class ToolboxToolset extends BaseToolset {
  ...
  async getTools(context?: ReadonlyContext): Promise<BaseTool[]> {
    ...
    // `isToolSelected` runs a predicate filter even without a context. This
    // toolset does not: its predicate is documented as context-dependent, so
    // it returns every tool instead of filtering on an absent context.
