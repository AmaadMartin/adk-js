...
/** The payload {@link executeSql} returns when the query succeeds. */
export interface ExecuteSqlPayload {
   ...
   * the query ran out of rows. Absent otherwise, as in adk-python.
   */
...
export function executeSql(client: Bigtable, options: ExecuteSqlOptions) {
  return runBigtableTool<ExecuteSqlPayload>('execute_sql', async () => {
    ...
    const payload: ExecuteSqlPayload = {rows: []};
    for await (const row of stream) {
      if (payload.rows.length >= maxRows) {
        payload.result_is_likely_truncated = true;
        break;
      }
      if (!isNamedList(row)) {
        throw new Error(
      ...
      payload.rows.push(namedListToJson(row));
    }
