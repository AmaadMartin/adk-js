...
export class LlmAgent extends BaseAgent<LlmAgentConfig> {
  ...
  private setModelResponseTool?: SetModelResponseTool;
  beforeModelCallback?: BeforeModelCallback;
  afterModelCallback?: AfterModelCallback;
  beforeToolCallback?: BeforeToolCallback;
  afterToolCallback?: AfterToolCallback;
  ...
  private async *runOneStepAsync(
    ...
    if (this.mode === 'task') {
      ...
      this.setModelResponseTool ??= new SetModelResponseTool(
        this.outputSchemaSource ?? this.outputSchema,
      );
      allTools.push(this.setModelResponseTool);
    }
  ...
  private async *postprocess(
    ...
    // A validated `set_model_response` call is the agent's answer, so promote
    // it to a model event, as adk-python's base_llm_flow does. A rejected call
    // publishes nothing, so the error reaches the model as an ordinary
    // function response and the loop runs another step -- the retry path.
