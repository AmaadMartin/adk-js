...
/**
 * Builds the transcription event without touching the session.
 *
 * The caller owns the returned event and decides whether to append it, yield it
 * or drop it, which mirrors `google/adk-python`'s
 * `TranscriptionManager._create_and_save_transcription_event`.
 */
...
/** Counts the transcriptions carried by the given events. */
function countTranscriptions(events: Event[]): TranscriptionStats {
 ...
 *
 * Ports `google/adk-python`'s `TranscriptionManager`. The manager is stateless
 * and free of side effects: it builds events and counts them, and never writes
 * to the session.
 */
export class TranscriptionManager {
   ...
   * @returns The event carrying the transcription.
   */
  ...
  getTranscriptionStats(
    ...
    return countTranscriptions(invocationContext.session.events);
  }
}
