...
import {getTransferTargets} from '../transfer_utils.js';
...
export class RequestConfirmationLlmRequestProcessor extends BaseLlmRequestProcessor {
  ...
  override async *runAsync(
    ...
    // just approved is refused as an unregistered tool. The tool is built the
    // same way that processor builds it, so the reachable targets it accepts
    // are the same ones the model was offered.
