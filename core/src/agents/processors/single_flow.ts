 ...
 * its tools.
 *
 * Transfer to another agent is not part of it: `LlmAgent` appends the
 * agent-transfer processor itself when the agent can transfer.
 *
 * Both lists are fresh arrays per instance, so a caller that appends to one
 * instance affects no other agent.
 */
