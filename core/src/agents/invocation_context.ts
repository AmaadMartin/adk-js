...
export class InvocationContext {
   ...
   *
   * Like the cost manager, the object is shared with every copy of this
   * context: {@link clone} and `BaseAgent.createInvocationContext` spread the
   * own fields over, so a sub-agent writes into the same record the parent
   * reads. Mirrors adk-python's `InvocationContext._custom_metadata`.
   */
