...
export const codeConfigSchema = z.preprocess(
  ...
  z.strictObject({
     ...
     * `my_library.my_callbacks.my_callback`. An eval config's custom metric
     * names its scoring function in the `<module specifier>#<export>` form
     * instead, such as `./metrics.js#score`.
     */
    name: z.string().min(1),
  }),
);
