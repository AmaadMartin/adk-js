...
describe('storage schema DDL', () => {
  ...
  it('emits MySQL datetime(6) for every temporal column', async () => {
    // The synchronous constructor loads the entity metadata without opening a
    // connection, so the schema generator can render the MySQL DDL offline.
    // MikroORM v7 dropped the `connect: false` option that `init` used to take.
    orm = new MikroORM({
      driver: MySqlDriver,
      entities: ENTITIES,
      dbName: 'adk',
