...
describe('DatabaseSessionService parity with adk-python', () => {
  ...
  describe('row-level locking', () => {
     ...
     * delta. adk-js never locks those two rows at all, so every case asserts
     * the stronger property, plus the dialect gate this change adds.
     */
    ...
    it.each(stateDeltas)(
      ...
      async (_id, stateDelta) => {
        ...
        try {
          ...
          expect(locked.get(StorageAppState)).toBeUndefined();
          expect(locked.get(StorageUserState)).toBeUndefined();
        } finally {
          findOne.mockRestore();
          await service.close();
        }
