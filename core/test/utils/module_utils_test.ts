...
import {staticProvider as exampleStaticProvider} from '../agents/fixtures/example_code_refs.js';
import {
  notAProvider,
  staticProvider,
...
describe('resolveFullyQualifiedName, against the example code refs fixture', () => {
  /** Absolute path of the module the qualified names below name. */
  const FIXTURE_PATH = fileURLToPath(
  ...
  it('reads the named export of an absolute specifier', async () => {
    ...
    ).resolves.toBe(exampleStaticProvider);
  });
