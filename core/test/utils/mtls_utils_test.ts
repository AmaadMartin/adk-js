...
describe('getApiEndpoint', () => {
  ...
  it('leaves a template with no placeholder unchanged', () => {
    ...
    ).toBe('https://agentregistry.mtls.googleapis.com/v1alpha');
  });
});
...
describe('getBytesWithClientCert', () => {
  const URL = 'https://agentregistry.mtls.googleapis.com/v1alpha/skills';
  ...
  /** Makes the mocked `https.request` answer with `chunks`. */
  function respondWith(chunks: Buffer[], statusCode?: number): void {
  ...
  it('presents the certificate and keeps the headers and the deadline', async () => {
    respondWith([Buffer.from('{"skills":[]}', 'utf-8')], 200);

    await expect(
      getBytesWithClientCert(URL, HEADERS, CERTS, TIMEOUT_MS),
    ).resolves.toEqual({
      status: 200,
      body: Buffer.from('{"skills":[]}', 'utf-8'),
    });
  ...
  // A zip archive holds bytes no UTF-8 decoder round-trips, so the transport
  // that serves the revision download must not decode at all.
  it('returns a binary body byte for byte', async () => {
    const zip = Buffer.from([0x50, 0x4b, 0x03, 0x04, 0x00, 0xff, 0xfe, 0x80]);
    respondWith([zip.subarray(0, 4), zip.subarray(4)], 200);

    const response = await getBytesWithClientCert(
      URL,
      HEADERS,
      CERTS,
      TIMEOUT_MS,
    ...
    expect(response.body.equals(zip)).toBe(true);
  });

  it('reports the status and the body of a failed response', async () => {
    respondWith([Buffer.from('forbidden', 'utf-8')], 403);

    await expect(
      getBytesWithClientCert(URL, HEADERS, CERTS, TIMEOUT_MS),
    ).resolves.toEqual({
      status: 403,
      body: Buffer.from('forbidden', 'utf-8'),
    });
  });
  ...
  it('reports a response with no status code as status 0', async () => {
    ...
    await expect(
      getBytesWithClientCert(URL, HEADERS, CERTS, TIMEOUT_MS),
    ).resolves.toEqual({status: 0, body: Buffer.alloc(0)});
  });
  ...
  it('destroys a request that reaches its deadline', async () => {
    ...
    const failure = getBytesWithClientCert(URL, HEADERS, CERTS, TIMEOUT_MS);
