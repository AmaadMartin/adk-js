...
function runTool(
  ...
  // tool call needs the drain loop `LlmAgent` runs around it.
  void (async () => {
    for await (const _event of drainInvocationEvents(queue)) {
      // The cases below assert on the tool result, not on the node's events.
    }
  })();
...
describe('NodeTool error paths', () => {
  ...
  it('throws when there is no function-call id', async () => {
    ...
    const ic = createIc();
    ic.eventQueue = new AsyncQueue<QueuedInvocationEvent>();
