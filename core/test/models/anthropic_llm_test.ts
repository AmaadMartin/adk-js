...
// Mock google-auth-library so the Claude (Vertex) tests can assert the outbound
// request without real Application Default Credentials. A plain class (rather
// than a `vi.fn()`) is used so the mock survives `vi.restoreAllMocks()`; the
// `let`s are read lazily so each test can reconfigure the credential behavior.
...
describe('resolveVertexModelId', () => {
  it('extracts the bare model id from a resource name', () => {
...
describe('buildVertexHost', () => {
  it('resolves the global, multi-region and regional hosts', () => {
...
describe('buildVertexEndpoint', () => {
  it('builds the rawPredict and streamRawPredict URLs', () => {
...
describe('resolveVertexConfig', () => {
  ...
  it('throws when project or location cannot be resolved', () => {
...
describe('Claude (Vertex)', () => {
  ...
  it('inherits the two supported model regexes', () => {
  ...
  it('uses the streamRawPredict endpoint when streaming', async () => {
    process.env[PROJECT_ENV] = 'env-project';
    process.env[LOCATION_ENV] = 'global';
    mockFetchStream([
      {
    ...
    await collect(new Claude().generateContentAsync(makeRequest(), true));
    expect(fetchedUrl()).toBe(
  ...
  it('resolves the claude-* patterns to Claude via the registry', () => {
  ...
  it('constructs a Claude instance via LLMRegistry.newLlm', () => {
    expect(LLMRegistry.newLlm('claude-3-5-sonnet-v2@20241022')).toBeInstanceOf(
      Claude,
    );
  });
});
