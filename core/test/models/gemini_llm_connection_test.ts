...
describe('GeminiLlmConnection', () => {
  ...
  describe('receive', () => {
    ...
    it('should flush pending transcription on interrupted', async () => {
      ...
      if (res1.done) {
        expect.fail('the connection yielded no partial input transcription');
      }
    ...
    it('should yield session resumption update', async () => {
      ...
      const resumptionUpdate = {newHandle: 'handle-1', resumable: true};
      messageQueue.push(
    ...
    it('should flush pending output transcription on interrupted', async () => {
      ...
      const res1 = await generator.next();
      if (res1.done) {
        expect.fail('the connection yielded no partial output transcription');
      }
