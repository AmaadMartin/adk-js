...
describe('GcsToolset', () => {
  ...
  it('exposes no write tool by default', async () => {
    const names = (await new GcsToolset().getTools()).map((t) => t.name);
  ...
  it('drops the prefix when the caller asks for none', async () => {
    ...
    const names = (await toolset.getTools()).map((t) => t.name).sort();
  ...
  it('matches a string toolFilter against the prefixed name', async () => {
    ...
    expect(await unprefixedFilter.getTools()).toEqual([]);
    expect((await prefixedFilter.getTools()).map((t) => t.name)).toEqual([
      'gcs_list_objects',
    ]);
  });
  ...
  it('applies a predicate toolFilter when a context is given', async () => {
    const toolset = new GcsToolset({
      toolFilter: (tool) => tool.name.endsWith('_list_objects'),
    });
    const context = new ReadonlyContext(makeInvocationContext());

    expect((await toolset.getTools(context)).map((t) => t.name)).toEqual([
      'gcs_list_objects',
    ]);
  });
  ...
  it('declares the model-facing parameters in snake_case', async () => {
    const tools = await new GcsToolset().getTools();
