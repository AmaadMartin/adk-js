...
describe('the completion details through the OpenTelemetry SDK', () => {
  it('exports one record correlated with the span, and the span attributes', async () => {
    ...
    // `@opentelemetry/sdk-logs` 0.222.0 keeps an attribute that is a list of
    // objects, so the message lists this module emits reach the exporter on the
    // `LogRecord` itself. The span below mirrors them as serialized strings.
    expect(record.attributes[GEN_AI_INPUT_MESSAGES]).toEqual([
      {
        role: 'user',
        parts: [{content: 'What is the weather?', type: 'text'}],
      },
    ]);
    expect(record.attributes[GEN_AI_OUTPUT_MESSAGES]).toEqual([
      {
        role: 'assistant',
