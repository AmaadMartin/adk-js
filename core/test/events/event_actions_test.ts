...
describe('createEventActions', () => {
  ...
  it('applies requestedAuthConfigs override', () => {
    const authConfig = makeAuthConfig('call-1');
    const actions = createEventActions({
    ...
    const result = mergeEventActions([
      {
        stateDelta: {},
        artifactDelta: {},
        requestedAuthConfigs: {'call-1': makeAuthConfig('call-1')},
        requestedToolConfirmations: {},
      },
      {
        stateDelta: {},
        artifactDelta: {},
        requestedAuthConfigs: {'call-2': makeAuthConfig('call-2')},
        requestedToolConfirmations: {},
      },
    ]);
    expect(result.requestedAuthConfigs).toEqual({
      'call-1': makeAuthConfig('call-1'),
      'call-2': makeAuthConfig('call-2'),
    });
  });
