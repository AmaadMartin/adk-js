...
describe('PluginManager.setSkipClosingPlugins', () => {
  it('should make close a no-op', async () => {
    const closeLog: string[] = [];
    const plugin = new ClosablePlugin('plugin1', closeLog);
    const manager = new PluginManager([plugin]);
    ...
    expect(plugin.closeCount).toEqual(0);
    expect(closeLog).toEqual([]);
  });

  it('should return before the close timeout can fire', async () => {
    const gate = createDeferred();
    const plugin = new GatedClosePlugin('hangs', gate.promise);
    const manager = new PluginManager([plugin], 1);
    manager.setSkipClosingPlugins(true);
  ...
  it('should close normally again once switched back off', async () => {
    const closeLog: string[] = [];
    const plugin = new ClosablePlugin('plugin1', closeLog);
    const manager = new PluginManager([plugin]);
    ...
    expect(closeLog).toEqual(['plugin1']);
  });
});
