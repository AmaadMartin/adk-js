...
/**
 * The provenance half of the node-tool resume path: the processor resumes only
 * work the agent itself started, and answers only interrupts the agent raised.
 *
 * A sibling of `request_input_llm_request_processor_test.ts`, which mocks
 * `handleFunctionCallList` away. These cases assert that the node really runs,
 * so they need the real one and therefore their own module registry.
 */
