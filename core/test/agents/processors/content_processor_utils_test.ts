...
describe('getContents with preserveFunctionCallIds', () => {
  ...
  it('is forwarded by getCurrentTurnContents', () => {
    ...
    expect(preserved[1].parts?.[0].functionCall?.id).toBe('adk-1');
    expect(preserved[2].parts?.[0].functionResponse?.id).toBe('adk-1');
    expect(stripped[1].parts?.[0].functionCall?.id).toBeUndefined();
    expect(stripped[2].parts?.[0].functionResponse?.id).toBeUndefined();
  });
});
