...
/** The transfer call the model issued, as the session recorded it. */
...
function createTransferConfirmationEvents(confirmed: boolean): Event[] {
  // The processor validates the confirmation against the call the model
  // issued, so the transfer call has to be in the history.
  const transferCallEvent = createEvent({
