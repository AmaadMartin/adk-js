...
describe('InvocationContext custom metadata', () => {
  it('starts a brand-new invocation with empty custom metadata', () => {
    const context = new InvocationContext({
      invocationId: 'inv-1',
      agent: new LoopAgent({name: 'root'}),
      session: makeSession(),
  ...
  it('shares the custom metadata with a cloned context', () => {
    const root = new InvocationContext({
      invocationId: 'inv-1',
      agent: new LoopAgent({name: 'root'}),
      session: makeSession(),
    ...
    const clone = root.clone({agent: new LoopAgent({name: 'sub'})});
    clone.customMetadata['source'] = 'sub-agent-tool';

    expect(root.customMetadata['source']).toBe('sub-agent-tool');
  });

  it('keeps custom metadata passed in at construction', () => {
    const context = new InvocationContext({
      invocationId: 'inv-1',
      agent: new LoopAgent({name: 'root'}),
      session: makeSession(),
      pluginManager: new PluginManager(),
      customMetadata: {source: 'caller'},
    });

    expect(context.customMetadata['source']).toBe('caller');
  });
});
