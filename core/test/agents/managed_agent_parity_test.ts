...
import {coerceToUserContent} from '@google/adk/utils/content_utils.js';
  ...
  describe('node input', () => {
    it('test_run_impl_bridges_node_input_to_user_content', async () => {
      ...
      expect(onlyCreateParams().input).toEqual(
        convertContentToSteps(coerceToUserContent('compute primes')),
      );
    });
    ...
    it('run_impl_without_node_input_keeps_the_parent_user_content', async () => {
      ...
      expect(onlyCreateParams().input).toEqual(
        convertContentToSteps(coerceToUserContent('from the session')),
      );
    });
  });
