...
// The barrel exports this schema as `declarativeLoopAgentConfigSchema`,
// because `agents/agent_config.ts` already holds the plain name there.
...
describe('parseLoopAgentYamlConfig', () => {
  ...
  // The shared `common_configs.ts` schema names the field the way adk-js
  // spells it, so these messages read `configPath` where adk-python reads
  // `config_path`.
  describe('sub-agent references', () => {
    ...
    it('rejects a reference naming neither code nor config_path', () => {
      ...
      ).toThrow(/Exactly one of `code` or `configPath` must be provided/);
    });
    ...
    it('rejects a reference naming both code and config_path', () => {
      ...
      ).toThrow(/Only one of `code` or `configPath` should be provided/);
    });
