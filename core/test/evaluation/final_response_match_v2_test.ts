...
describe('FinalResponseMatchV2Evaluator', () => {
  ...
  it.each([
    ...
    expect(evaluator.convertAutoRaterResponseToScore(critique)).toEqual({
      score,
    });
  });
  ...
  it.each(['invalid json', '{}', ''])(
    ...
    (text) => {
      ...
      // The metric is not rubric based, so an unscored critique yields an
      // auto-rater score carrying neither a score nor rubric scores.
      expect(
