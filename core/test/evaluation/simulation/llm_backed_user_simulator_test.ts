...
/**
 * Covers the config of the LLM-backed user simulator, and ports
 * `tests/unittests/evaluation/simulation/test_llm_backed_user_simulator.py`
 * from google/adk-python at commit baf7efbaa92c. Each ported `it` keeps the
 * name of the Python test it ports.
 */
...
/** Instructions naming every placeholder the config shape requires. */
const CUSTOM_INSTRUCTIONS =
  'Be brief. Stop with {{ stop_signal }}. Follow {{ conversation_plan }} ' +
...
describe('parseLlmBackedUserSimulatorConfig', () => {
  it('reads a field written in the adk-python spelling', () => {
    ...
    expect(config.customInstructions).toBe(CUSTOM_INSTRUCTIONS);
