...
describe('parseEvalConfig, beyond the reference tests', () => {
  ...
  it('keeps a key a user simulator config does not name', () => {
    ...
    // A key the shape does not name keeps the spelling the document wrote,
    // because only a declared field carries an alias to rename it through.
    expect(config.userSimulatorConfig).toMatchObject({my_own_setting: 'kept'});
  });
  ...
  it('rejects a userSimulatorConfig whose type is not a string', () => {
    expect(() =>
      ...
      /names a user simulator of type 7\. The supported types are 'llm_backed' and 'llm_audio'/,
    );
  });
