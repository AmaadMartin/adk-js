...
describe('LlmAsJudge.evaluateInvocations', () => {
  ...
  it('sends one prompt per invocation, shared by that invocation samples', async () => {
    ...
    // The judge that answers, not the one the criterion names: a caller that
    // supplies a judge model must have the request routed to it.
    expect(judgeModel.requests[0].model).toBe('fake-judge-model');
...
function createMatchV2Evaluator(
...
function createResponseInvocation(finalResponse: string): Invocation {
  return {
...
function createInvocations(count: number): Invocation[] {
  return Array.from({length: count}, (_, index) =>
    createResponseInvocation(`response ${index}`),
  );
}
...
function evaluate(
  ...
  const evaluator = createMatchV2Evaluator(judgeModelOptions, judgeModel);
...
describe('LlmAsJudge', () => {
  ...
  it('rejects an eval case with no reference invocations', async () => {
    const evaluator = createMatchV2Evaluator(
      {},
  ...
  // `numSamples: 0` is not listed here: adk-python puts `ge=1` on
  // `parallelism_limit` alone, so a judge that samples nothing is legal and
  // simply grades no invocation. See 'omits an invocation that is never
  // sampled' above.
  it.each([{parallelismLimit: 0}])(
    'rejects the judge options %j',
    (judgeModelOptions) => {
      expect(() =>
        createMatchV2Evaluator(
          judgeModelOptions,
          new FakeJudgeLlm([{silent: true}]),
        ),
