...
describe('Runner app name alignment', () => {
  ...
  it('explains a mismatched app name in the session-not-found message', async () => {
    const runner = createRunnerWithOrigin(
      {appName: 'loaded_app', path: '/agents/loaded_app'},
      sessionService,
    );
  ...
  it('reads back the origin a loader recorded', () => {
    const agent = new EchoAgent();
    const origin = {appName: 'stamped_app', path: '/agents/stamped_app'};

    stampAgentOrigin(agent, origin);

    expect(inferAgentOrigin(agent)).toEqual(origin);
  });

  it('reports no origin for an unstamped agent', () => {
    expect(inferAgentOrigin(new EchoAgent())).toEqual({});
  });
});
