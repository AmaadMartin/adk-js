...
describe('mergeLiveEventStreams', () => {
  it('surfaces events from both streams', async () => {
    ...
    queue.push({event: testEvent('queued')});
  ...
  it('releases the producer of a queued event once the caller has taken it', async () => {
    const queue = new AsyncQueue<QueuedInvocationEvent>();
    const markProcessed = vi.fn();
    queue.push({event: testEvent('queued'), markProcessed});
    const merged = mergeLiveEventStreams(queue, eventsOf(testEvent('root')));
