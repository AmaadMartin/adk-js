...
import type {HttpDebugExchange} from '../../../src/tools/mcp/http_debug_recorder.js';
...
describe('MCPToolset HTTP debug capture', () => {
  ...
  describe('with debug logging on', () => {
    ...
    it('records the exchange onto the invocation', async () => {
      ...
      expect(exchanges?.[0].status_code).toBe(200);
      expect(exchanges?.[0].request_headers['authorization']).toBe(
  ...
  describe('with debug logging off', () => {
    it('leaves the invocation untouched and installs no recording fetch', async () => {
      ...
      expect(sdk.transportOptions?.fetch).toBe(callerFetch);
    });
  });
});
