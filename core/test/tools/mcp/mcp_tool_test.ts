...
// The HTTP debug recorders are internal (not part of the public API), as is
// the logger singleton, so all are imported via a relative path.
import {
  getHttpDebugSink,
  HttpDebugExchange,
  mcpHttpDebugStorage,
...
function createStubSessionManager(...clients: Client[]): MCPSessionManager {
  const manager = new MCPSessionManager({
    type: 'StdioConnectionParams',
    serverParams: {command: 'unused'},
  });
...
describe('MCPTool.rawMcpTool', () => {
  it('returns the tool definition the constructor received', () => {
    ...
    const tool = new MCPTool(mcpTool, createStubSessionManager());
...
describe('MCPTool.mcpAppResourceUri', () => {
  ...
  it('returns undefined when the tool declares no _meta', () => {
    const tool = new MCPTool(createMcpTool(), createStubSessionManager());
...
describe('MCPTool UI widget', () => {
  ...
  it('renders an MCP App widget after a successful call', async () => {
    ...
    const tool = new MCPTool(
      mcpTool,
      createStubSessionManager(createStubClient()),
    );
  ...
  it('renders no widget when the call fails', async () => {
    ...
    const tool = new MCPTool(
      createMcpTool({ui: {resourceUri: 'ui://test-app'}}),
      createStubSessionManager(client),
    );
    const toolContext = createToolContext({functionCallId: 'test-call-id'});
...
describe('MCPTool trace context propagation', () => {
  ...
  it('sends the active trace context in the call _meta', async () => {
    ...
    const client = createStubClient();
    const tool = new MCPTool(createMcpTool(), createStubSessionManager(client));
    const toolContext = createToolContext();
...
describe('MCPTool session retry', () => {
  ...
  it('opens a second session when the first one fails to open', async () => {
    const succeeding = createStubClient();
    const manager = createStubSessionManager();
    vi.mocked(manager.createSession)
  ...
  it('never replays a call the server may have received', async () => {
    ...
    const manager = createStubSessionManager(client);
    const tool = new MCPTool(createMcpTool(), manager);
 ...
 */
function makeClientSessionManager(client: Client): MCPSessionManager {
  const manager = new MCPSessionManager({
    type: 'StdioConnectionParams',
...
describe('MCPTool.rawMcpTool', () => {
  it('returns the exact object passed to the constructor', () => {
    ...
    const tool = new MCPTool(
      mcpTool,
      makeClientSessionManager(makeClient(vi.fn())),
    );
...
describe('MCPTool.mcpAppResourceUri', () => {
  function uriFor(meta?: unknown): string | undefined {
    const sessionManager = makeClientSessionManager(makeClient(vi.fn()));
...
describe('MCPTool UI widget rendering', () => {
  ...
  it('renders one widget after a successful call', async () => {
    ...
    const tool = new MCPTool(mcpTool, makeClientSessionManager(client));
  ...
  it('renders nothing for a tool that declares no resource URI', async () => {
    const client = makeClient(vi.fn().mockResolvedValue({content: []}));
    const tool = new MCPTool(makeMcpTool(), makeClientSessionManager(client));
    const toolContext = contextWith('test-call-id');
...
describe('MCPTool trace context propagation', () => {
  ...
  async function callToolParams(): Promise<Record<string, unknown>> {
    ...
    const tool = new MCPTool(
      makeMcpTool(),
      makeClientSessionManager(makeClient(callTool)),
    );
