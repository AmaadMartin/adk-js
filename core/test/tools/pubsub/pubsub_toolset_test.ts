...
import {getPublisherClient, getSubscriberClient, cleanupClients} from '../../../src/tools/pubsub/client.js';
...
describe('PubSubToolset', () => {
  ...
  describe('publishMessage tool', () => {
    it('should successfully publish a message', async () => {
      const tools = await toolset.getTools();
      const publishTool = tools.find((t) => t.name === 'publish_message') as any;
      
      const pubClient = getPublisherClient();
      vi.mocked(pubClient.publish).mockResolvedValue([{messageIds: ['msg-123']}]);
      ...
      expect(pubClient.publish).toHaveBeenCalledWith(expect.objectContaining({
        topic: 'test-topic',
      }));
    });
    ...
    it('should return ERROR status when publish fails', async () => {
      ...
      const pubClient = getPublisherClient();
      vi.mocked(pubClient.publish).mockRejectedValue(new Error('Publish timeout'));
  ...
  describe('pullMessages tool', () => {
    it('should successfully pull messages', async () => {
      const tools = await toolset.getTools();
      const pullTool = tools.find((t) => t.name === 'pull_messages') as any;
  ...
  describe('acknowledgeMessages tool', () => {
    it('should successfully acknowledge messages', async () => {
      const tools = await toolset.getTools();
      const ackTool = tools.find((t) => t.name === 'acknowledge_messages') as any;
    ...
    it('should return ERROR status when acknowledge fails', async () => {
        ...
        const subClient = getSubscriberClient();
        vi.mocked(subClient.acknowledge).mockRejectedValue(new Error('Ack timeout'));
