...
/**
 * What a developer sees when `@google-cloud/pubsub` is not installed.
 *
 * Mocking `@google-cloud/pubsub` itself cannot produce this: Vitest replaces
 ...
 * it stays separate from the other Pub/Sub tests.
 */
...
describe('the Pub/Sub client module without its peer dependency', () => {
  it.each([
    {label: 'the publisher', get: getPublisherClient},
    {label: 'the subscriber', get: getSubscriberClient},
  ])(
