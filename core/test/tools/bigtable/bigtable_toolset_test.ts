...
describe('BigtableToolset', () => {
  ...
  it('reports a failed call as an error result rather than throwing', async () => {
    ...
    expect(result).toEqual({
      ...
      error_details: "Error in tool 'bigtable_execute_sql': invalid query",
    });
  });
  ...
  // `FunctionTool` on this branch answers a missing mandatory argument with a
  // retry hint, as adk-python does, and returns before `GoogleTool` sees a
  // thrown error to convert. The call still comes back as a result rather than
  // as a throw, which is what this case guards.
  it('rejects a call that omits a mandatory argument', async () => {
    const [listInstancesTool] = await new BigtableToolset({
      toolFilter: ['list_instances'],
    }).getTools();
    ...
    expect(result).toMatchObject({error: expect.stringContaining('projectId')});
  });
