...
/** Asserts the tool succeeded and narrows the result to its success payload. */
export function expectSuccess<T extends object>(
  result: BigtableToolResult<T>,
): {status: 'SUCCESS'} & T {
  if (result.status === 'SUCCESS') {
    return result;
  }
  expect.fail(`Expected SUCCESS but got ERROR: ${result.error_details}`);
}

/** Asserts the tool failed and narrows the result to its error message. */
export function expectErrorDetails<T extends object>(
  result: BigtableToolResult<T>,
): string {
  if (result.status === 'ERROR') {
    return result.error_details;
  }
  expect.fail(`Expected ERROR but got SUCCESS: ${JSON.stringify(result)}`);
}
