...
describe('Bigtable Query Tool', () => {
  ...
  it('returns rows at the top level with no truncation key', async () => {
    instance.createExecuteQueryStream.mockReturnValue(
      fakeQueryStream([
        fakeNamedList([
          ['col1', 'val1'],
    ...
    const result = await queryTool.executeSql(client, {
      instanceId: 'inst',
  ...
  it('returns an empty row list for an empty result set', async () => {
    instance.createExecuteQueryStream.mockReturnValue(fakeQueryStream([]));
  ...
  it('caps the result at 50 rows when no settings are supplied', async () => {
    instance.createExecuteQueryStream.mockReturnValue(
      fakeQueryStream(
        Array.from({length: 51}, (unused, id) => fakeNamedList([['id', id]])),
      ),
    );
    ...
    expect(result.rows.at(-1)).toEqual({id: 49});
    expect(result.result_is_likely_truncated).toBe(true);
  });

  it('reports the bare error message without the Error prefix', async () => {
    instance.prepareStatement.mockRejectedValue(new Error('Test error'));
