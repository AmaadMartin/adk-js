...
describe('normalizeSchema', () => {
  ...
  it('should parse a JSON string schema', () => {
    expect(normalizeSchema('{"type":"string"}', 'response body')).toEqual({
      type: 'string',
    });
  });

  it('should reject a string that is not JSON', () => {
    expect(() => normalizeSchema('not json', 'response body')).toThrow(
      'response body must be an OpenAPI schema, got invalid JSON',
    );
  });
  ...
  it('should reject a JSON string that is not an object', () => {
    ...
    expect(() => normalizeSchema('[1,2]', 'response body')).toThrow(
...
describe('createApiParameter', () => {
  ...
  it('should normalize a JSON string schema', () => {
    const param = createApiParameter({
      originalName: 'param1',
      paramLocation: 'query',
      paramSchema: '{"type":"integer"}',
    ...
    expect(param.paramSchema).toEqual({type: 'integer'});
    expect(getTypeHint(param.paramSchema)).toBe('number');
  });
