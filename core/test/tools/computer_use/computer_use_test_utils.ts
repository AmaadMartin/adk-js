...
export class MockComputer extends BaseComputer {
  ...
  async clickAt({x, y}: {x: number; y: number}): Promise<ComputerState> {
    return state(`${MOCK_PAGE_URL}/click/${x}/${y}`);
  }

  async hoverAt({x, y}: {x: number; y: number}): Promise<ComputerState> {
  ...
  async wait({seconds}: {seconds: number}): Promise<ComputerState> {
  ...
  async navigate({url}: {url: string}): Promise<ComputerState> {
    this.navigateCalls.push(url);
    return state(url);
  }

  async keyCombination({keys}: {keys: string[]}): Promise<ComputerState> {
