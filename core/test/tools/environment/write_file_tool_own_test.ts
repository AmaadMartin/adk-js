...
describe('WriteFileTool', () => {
  ...
  it('declares a path and a content argument as a JSON schema', () => {
    const tool = new WriteFileTool(new StubEnvironment());
  ...
  it('detects no error in a response that is not a status object', () => {
    const tool = new WriteFileTool(new StubEnvironment());
    ...
    expect(tool.detectErrorInResponse([{status: 'error'}])).toBeUndefined();
  });
});
