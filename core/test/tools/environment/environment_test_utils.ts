...
/**
 * Builds a tool context backed by real `InvocationContext`, `LlmAgent` and
 * session objects, so the fixture breaks if their contracts change rather than
 * being silenced by a cast.
 */
...
/** A tool context whose call the client has already approved. */
...
/** A tool context whose call the client has refused. */
