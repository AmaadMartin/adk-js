 ...
 * `tests/unittests/tools/environment/test_read_file_tool.py`. Each `it()` name
...
class StubEnvironment extends BaseEnvironment {
  ...
  override get workingDir(): string {
    return path.join(os.tmpdir(), 'adk-test');
  }
  ...
  override async writeFile(): Promise<void> {
    throw new Error('StubEnvironment does not support writeFile().');
  }
}
...
describe('TestReadFileTool', () => {
  ...
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'adk-read-file-test-'));
    env = new LocalEnvironment({workingDir: tmpRoot});
  ...
  it('test_read_file_rejects_boolean_line_numbers', async () => {
    ...
    const resStart = await tool.runAsync({
      args: {path: 'sample.txt', start_line: true},
      toolContext: createToolContext(),
    });
    const resEnd = await tool.runAsync({
