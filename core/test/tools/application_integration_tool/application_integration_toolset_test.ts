...
import {DEFAULT_OPENAPI_CREDENTIAL_KEY} from '../../../src/tools/openapi_tool/openapi_spec_parser/tool_auth_handler.js';
...
describe('ApplicationIntegrationToolset', () => {
  ...
  describe('connection mode', () => {
    ...
    it('honours a tool filter given as a name list', async () => {
      const tools = await createToolset({
        toolFilter: ['jira_create_issues'],
      }).getTools();

      expect(tools.map((tool) => tool.name)).toEqual(['jira_create_issues']);
    });
  ...
  describe('exchanged end-user auth', () => {
    ...
    it('clones for an OAuth2 configuration too', async () => {
      ...
      expect(toolset.getAuthConfig()).toEqual({
        ...
        credentialKey: DEFAULT_OPENAPI_CREDENTIAL_KEY,
    ...
    it('filters before it clones', async () => {
      const toolset = createToolset({
        entityOperations: {Issues: ['list', 'create']},
        toolFilter: ['jira_list_issues'],
        authScheme: END_USER_SCHEME,
        authCredential: END_USER_CREDENTIAL,
      });
      const stored = await toolset.getTools();
      expect(stored.map((tool) => tool.name)).toEqual(['jira_list_issues']);
      ...
      expect(resolved[0].name).toBe('jira_list_issues');
    });
  ...
  describe('the generated spec through to the outgoing request', () => {
    ...
    beforeEach(() => {
      fetchStub = vi.fn().mockResolvedValue({
        ...
        text: () =>
          Promise.resolve(JSON.stringify({connectorOutputPayload: {id: '1'}})),
      });
