 ...
 * told to, so a teardown can be observed step by step. `stdinFails` makes the
 * write report EPIPE, as a child that died before reading the program does.
 */
function createFakeChild(pid: number, signalled: string[], stdinFails = false) {
...
describe('UnsafeLocalCodeExecutor', () => {
  ...
  describe('exit status classification', () => {
    it('does not fail a python run that wrote to stderr and exited 0', async () => {
      ...
      // The status, not the warning, says whether the run failed, so the
      // warning stays in stderr for the caller to read.
    ...
    it('does not fail a javascript run that wrote to stderr and exited 0', async () => {
      const result = await executor.executeCode({
        invocationContext,
        codeExecutionInput: {
      ...
      expect(result.stderr).toContain('a deprecation warning');
      expect(result.exitCode).toBe(0);
      expect(buildCodeExecutionResultPart(result).codeExecutionResult).toEqual({
        outcome: Outcome.OUTCOME_OK,
      });
