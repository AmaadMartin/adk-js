...
describe('RemoteA2AAgent request interceptors', () => {
  ...
  it('sends no service parameters when no interceptor supplies headers', async () => {
    const {agent, transport} = createAgent([agentMessage('hi')]);

    await drain(agent, createContext());

    // The agent always passes request options, because they carry the timeout
    // signal. Only the service parameters are interceptor-supplied.
    expect(transport.sentOptions[0]?.serviceParameters).toBeUndefined();
  });
  ...
  it('turns a rejecting hook into an error event', async () => {
    ...
    expect(events[0].errorMessage).toBe('A2A request failed: hook failed');
  });
});
