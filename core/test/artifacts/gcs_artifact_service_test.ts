...
describe('GcsArtifactService', () => {
  ...
  describe('ArtifactVersion fields', () => {
    ...
    /** Puts the fixed creation time on a stored object, which the fake stamps
     * with the current time. */
    function stampCreateTime(objectName: string): void {
      const entry = storageMock.bucket(bucketName).files.get(objectName);
      if (!entry) {
        expect.fail(`Expected ${objectName} to be stored.`);
      }
      entry.timeCreated = TIME_CREATED;
    }
    ...
    it('reports a gs:// canonical URI and a creation time in seconds', async () => {
      ...
      stampCreateTime('test-app/test-user/test-session/notes.txt/0');
    ...
    it.each([undefined, 'not a timestamp'])(
      'falls back to the current time when the object reports %s',
      async (timeCreated) => {
        const service = createService();
        storageMock
          .bucket(bucketName)
        ...
        // The record always carries a time, so an unusable one reads as now
        // rather than as a number parsed out of the bad value.
        expect(artifactVersion?.createTime).toBeLessThanOrEqual(
          Date.now() / 1000 + 1,
        );
        expect(artifactVersion?.createTime).toBeGreaterThan(0);
      },
    );
    ...
    it('lists one record per version, ascending', async () => {
      ...
      stampCreateTime('test-app/test-user/test-session/notes.txt/0');
      stampCreateTime('test-app/test-user/test-session/notes.txt/1');
