...
import {createArtifactVersion, InMemoryArtifactService} from '@google/adk';
...
describe('InMemoryArtifactService user-namespace listing', () => {
  ...
  it('rejects an empty session id instead of treating it as an omitted one', async () => {
    const service = new InMemoryArtifactService();
...
describe('InMemoryArtifactService public artifacts store', () => {
  ...
  it('exposes the saved part and its recorded metadata', async () => {
    ...
    expect(entry.artifactVersion).toEqual({
      version: 0,
      canonicalUri:
        'memory://apps/app0/users/user0/sessions/s0/artifacts/note.txt/versions/0',
      customMetadata: {origin: 'test'},
    ...
    service.artifacts['user/app0/user0/user%3Aprofile.txt'] = [
      {
        ...
        artifactVersion: createArtifactVersion({
          version: 0,
          canonicalUri:
            'memory://apps/app0/users/user0/artifacts/user:profile.txt/versions/0',
        }),
      },
    ];
