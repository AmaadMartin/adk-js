...
import type {BasePlanner, LlmRequest} from '@google/adk';
import {
  BuiltInPlanner,
  Context,
  InvocationContext,
...
/** A third-party planner, proving `BasePlanner` is implementable elsewhere. */
class CustomPlanner implements BasePlanner {
  buildPlanningInstruction(): string {
    return 'plan first';
  }

  processPlanningResponse(): Part[] {
    return [];
  }
}
...
describe('BuiltInPlanner', () => {
  ...
  it('processes no planning response and does not mutate the parts', () => {
    ...
    expect(
      planner.processPlanningResponse(callbackContext, responseParts),
    ).toBeUndefined();
