...
An agent you construct yourself carries no origin, so the hint never fires for it and the message stays `Session not found: <id>`. To supply an origin from your own loader, call `stampAgentOrigin`:

```typescript
import {stampAgentOrigin} from '@google/adk';

stampAgentOrigin(agent, {appName: 'weather_bot', path: '/agents/weather_bot'});
```
