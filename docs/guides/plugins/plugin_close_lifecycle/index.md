...
`BasePlugin.close()` is the hook for that shutdown. The default does nothing, so a plugin that holds no resource needs no override. `PluginManager.close()` calls the hook on every registered plugin, and `Runner.close()` calls the manager after it has closed the toolsets.
...
- `Runner.close()` is idempotent. A second call does nothing, so each plugin closes exactly once.
