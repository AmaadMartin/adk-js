...
model's `responseParts`, and returns the parts to keep. Each member takes one
params object, so a new member can be added without breaking a subclass. Both
members may return `undefined`, which means "nothing to contribute" — a caller
treats it as a no-op, not as an error. Both may also return a promise.
...
`@google/adk` ships two concrete planners, `BuiltInPlanner` and
`PlanReActPlanner`, and the NL-planning request processor drives them. This
class is the base that a planner extends, and `isBasePlanner` is how a caller
finds one. Both are also usable by code that drives a planner itself, as the
example below does.
...
async function plan(
  ...
  const kept = await planner.processPlanningResponse({
