...
when the judge model has a small quota. A `parallelismLimit` below 1 is
rejected when the evaluator is constructed. A `numSamples` of 0 is accepted,
as it is in adk-python, and the evaluator then grades no invocation.
