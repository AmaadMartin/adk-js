...
whole conversation. Reach for it when the agent reaches the goal, but you need
to grade how it got there.
...
## Describing the agents and the route

The service scores the route, so give it the route. Each `Invocation` carries
two optional fields the request maps:

- `intermediateData.invocationEvents` are the steps between the user's message
  and the reply, author and content preserved in order.
- `appDetails.agentDetails` describes the agents that served the turn. The
  first turn that declares an agent describes it; a later turn that declares
  the same name again is ignored.
    ...
    invocationEvents: [
      {
        ...
        content: {parts: [{text: 'calling get_forecast'}]},
      },
    ],
  },
  finalResponse: {parts: [{text: 'Rain, 15 degrees.'}]},
...
turn's score, and `overallEvalStatus` is `PASSED` at or above the threshold.
...
result and sends no request. A rejection from your client propagates unchanged.
...
- `adk-python` builds a Vertex AI client from `GOOGLE_CLOUD_PROJECT` and
  ...
  credentials.
- `adk-python` registers this evaluator in its metric evaluator registry. ADK
  TypeScript ships no such registry yet, so construct the evaluator yourself.
