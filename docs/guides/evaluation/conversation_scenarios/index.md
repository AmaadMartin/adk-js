...
more than `JSON.stringify`.
`conversationScenariosModel.dump(value, {byAlias: true})` writes the
snake_case names adk-python reads instead. Both spellings parse back
