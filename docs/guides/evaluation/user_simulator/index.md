...
replaces the provider entirely. See
[Driving a case yourself](#driving-a-case-yourself).
...
Pass `createUserSimulator` to `generateResponses` to replace the provider. Use
this when one eval run needs a simulator you do not want in the process-wide
registry.
