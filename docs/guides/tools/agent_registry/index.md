...
`getRemoteA2AAgent` resolves no binding. `RemoteA2AAgent` cannot apply a
credential to its outgoing calls, so a resolved scheme would cost a request and
change nothing. Pass an already-authenticated `client` instead.
