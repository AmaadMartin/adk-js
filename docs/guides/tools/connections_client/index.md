...
output schemas of an action. Reach for it when you build a tool over a
connector and need to know what the connector accepts before you call it.

## Introduction

An Integration Connectors connection exposes entities and actions whose shape is
decided by the connector, not by your code. A Jira connection publishes
different entities from a Salesforce one, and the fields of each entity are only
known at runtime. A tool that wants to call `ExecuteConnection` therefore has to
ask the Connectors API what exists first.

`ConnectionsClient` is that read path. It calls
`connectors.googleapis.com`, waits for the long-running operation each metadata
call starts, and returns plain objects. It never writes and never executes a
connector call.
...
`getActionOperation()` builds one for an action. `convertJsonSchemaToOpenApiSchema()`
converts a connector JSON schema into the subset OpenAPI 3.0 can express.

These builders have no caller inside this package yet. They are public API, and
`IntegrationClient` consumes them.
...
const client = new ConnectionsClient({
  ...
  location: 'us-central1',
  connection: 'my-connection',
  serviceAccountJson: await readFile(keyPath, 'utf8'),
});
```
...
The generated path item carries the connector call on `x-operation`, so a
toolset reading the spec can rebuild the request:
`spec.paths[path].post['x-operation']` is `'LIST_ENTITIES'`, and
`post.operationId` is `'issues_tool_list_Issues'`.

`getConnectorBaseSpec()` returns a fresh document on every call, so mutating
`paths` and `components.schemas` on one spec never affects another.
