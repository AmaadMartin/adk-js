...
One hook changes that behaviour. `processArtifact` replaces the built-in
conversion entirely, so an application can redact an artifact or drop it before
...
6. A text-like payload becomes its decoded text. The tool tries this when the
   ...
   `.json` or `.xml`.
7. Anything left becomes `[Binary artifact: <name>, type: <mime>, size: <n> KB.
