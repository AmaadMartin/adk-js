...
```ts
import {getHttpDebugInfo, LogLevel, setLogLevel} from '@google/adk';

setLogLevel(LogLevel.DEBUG);
await toolset.getTools(readonlyContext);

const exchanges = getHttpDebugInfo(
...
- The capture stops at 100 exchanges per invocation, so a long streaming session
