...
A request with no model opens none of the three, so the tool throws. Set
`isManagedAgent` on a request that carries no model by design.
...
- The request names no model, or a model that is not Gemini, and neither the
  environment variable nor `isManagedAgent` applies. The message is `Google
search tool is not supported for model <model>`.
