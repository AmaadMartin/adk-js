...
string is not a valid session id, and the service rejects it.
...
Build the metadata with `createArtifactVersion`. It fills in the fields a save
normally sets, such as the creation time.
