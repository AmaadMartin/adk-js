...
- [Rubric based multi-turn trajectory quality v1](evaluation/rubric_based_multi_turn_trajectory_quality_v1/index.md) - The LLM-as-a-judge metric that scores an agent's whole conversation against a list of written rubrics.
...
- [FirestoreSessionService](integrations/firestore_session_service/index.md) -
  The `@google/adk-integrations` Firestore session store, its revision-checked
...
- [WriteFileTool](tools/environment_write_file/index.md) - Letting an agent create or overwrite a file in a `BaseEnvironment` working directory.

### Workflow

- [BaseNode](workflow/base_node/index.md) - Node names, static node paths, and how a node's output is serialized onto an event.
- [Event branches in a node run](workflow/node_branches/index.md) - Which branch a node's events carry, and how a node redirects it.
