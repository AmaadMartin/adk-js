...
`llm_responses`. The adk-js recorders write the same list, and older adk-js
recordings carry a single `llmResponse`. Both fields are present on
`LlmRecordingSchema`, and `ReplayPlugin` reads both. It returns `llmResponse`
when it is set. Otherwise it returns the complete entry of `llmResponses`, since
a streamed turn is recorded as its partial responses followed by the complete
one.

A `beforeModelCallback` answers one model call with one response, so adk-js
cannot replay a recording that holds several complete responses. That recording
is refused by name:
