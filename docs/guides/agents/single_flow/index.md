...
another agent is not part of it: `AutoFlow` extends `SingleFlow` with
`AGENT_TRANSFER_LLM_REQUEST_PROCESSOR`, and `LlmAgent` builds an `AutoFlow`
when the agent has sub-agents or may transfer to a parent or a peer.
...
`responseProcessors` holds the NL planning response processor, which unmarks the
previous turn's thought parts, followed by the code execution response
processor, which runs the code the model emitted.

## Context compaction

Pass compactors to the constructor and a `ContextCompactorRequestProcessor`
evaluates them immediately before the contents processor, so the compacted
history is what the model sees. The processor is always inserted, because an
agent that declares no compactors still honours the compaction policy its App
