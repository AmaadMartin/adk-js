...
`model.capabilities.outputSchemaAndTools` decides which path a run takes. A
Gemini model declares it only on the Vertex AI variant at version 2.0 or later.
On that path the basic request processor sets `config.responseSchema`, the model
answers with JSON text, and nothing on this page applies.
...
The processor runs near the end of the agent's request-processor list, before
the tool filter. The filter narrows `allowedTools`, which the declared tool does
not go through, so the tool is never filtered away. A `task`-mode agent is left
alone, because it already returns its structured result through `finish_task`.
