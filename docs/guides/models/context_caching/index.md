...
A `CacheMetadata` record is in one of two states.
...
`isActiveCacheMetadata(metadata)` narrows a record to the active state.
`createCacheMetadata` builds one, and it rejects a half-populated record.
