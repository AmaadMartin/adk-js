...
exports.rootAgent = new NodeModulesAgent();`,
  ...
  it(
    'should load a directory agent from its index entrypoint',
    async () => {
  ...
  it(
    'should fall through an entrypoint that holds no agent',
    async () => {
      const agentFile = await loader.getAgentFile('service_helpers');
      const rootAgent = await agentFile.loadAgent();
