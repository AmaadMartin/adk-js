...
/** The child the server spawned, narrowed for the assertions that follow. */
function childOf(server: ScriptedTestServer): ChildProcessWithoutNullStreams {
  const child = server.child;
  if (!child) expect.fail('the server never spawned a child');
  return child;
}
...
describe('BaseTestServer.startProcess', () => {
  it('reserves the port before the child is spawned', async () => {
    ...
    // Nothing is chosen up front. A port the constructor invented would be one
    // no listener ever accepted, which is the failure this fix removes.
  ...
  it('surfaces the captured output on a start-up timeout', async () => {
    ...
    await server.stop();
    expect(hasExited(childOf(server))).toBe(true);
  });
